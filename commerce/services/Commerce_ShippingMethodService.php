 ...
 * @since     1.0
 */
class Commerce_ShippingMethodService extends BaseApplicationComponent
{
	/**
	 * @param int $id
	 *
	...
	)
	{
		...
		{
			if (craft()->commerce_shippingRule->matchOrder($rule, $order))
			{
				return $rule;
			}
		}
	...
	public function delete ($model)
	{
		...
		try
		{
			...
			$rules = craft()->commerce_shippingRule->getAllByMethodId($model->id);
			foreach ($rules as $rule)
			{
				craft()->commerce_shippingRule->deleteById($rule->id);
			}
