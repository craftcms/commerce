...
/**
 * Class Commerce_CustomerService
 *
 ...
 * @since     1.0
 */
class Commerce_CustomerService extends BaseApplicationComponent
{
	...
	public function saveAddress (Commerce_AddressModel $address)
	{
		...
		return craft()->commerce_address->saveAddress($address);
	}
	...
	{
		$addresses = craft()->commerce_address->getAddressesByCustomerId($customerId);
		$ids = [];
	...
	public function consolidateOrdersToUser ($username)
	{
		...
		try
		{
			...
			$orders = craft()->commerce_order->getByEmail($toCustomer->email);
