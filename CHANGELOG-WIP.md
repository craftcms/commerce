...
- Fixed a TODO in `craft\commerce\Plugin::_registerCraftEventListeners()`: the customer-attach bulk optimization (pre-populating `primaryBillingAddressId`/`primaryShippingAddressId` for a batch of `User` elements, so `User::getPrimaryBillingAddressId()`/`getPrimaryShippingAddressId()` — see `Plugin::registerCustomerMacros()` — don't each run their own per-user query) had been left commented out since the legacy `craft\elements\db\UserQuery::EVENT_AFTER_POPULATE_ELEMENTS` it depended on doesn't exist in Craft 6. Ported the logic to a new `Customers::elementsHydratedHandler()` method, listening for `CraftCms\Cms\Element\Queries\Events\ElementsHydrated` instead — the new event isn't scoped to `UserQuery` like the old one was (it fires for every element type's query), so the handler filters `$event->elements` for `User` instances itself. Registered declaratively via a new `Plugin::$events` array (`CraftCms\Cms\Plugin\Concerns\HasListeners`, already inherited by every plugin via `CraftCms\Cms\Plugin\Plugin` but not yet used by Commerce) rather than a closure in `registerCraftEventListeners()`, using Laravel's `'ClassName@methodName'` listener-string syntax so the handler can live as a normal named method on the `Customers` service instead of an anonymous closure or a dedicated listener class.
