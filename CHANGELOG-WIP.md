...
- Added `Order::getObjectTemplateVariables()`, ported from 5.x (fixes [#4255](https://github.com/craftcms/commerce/issues/4255)). `Order::fields()` re-serializes datetime attributes into `['date' => ..., 'time' => ...]` arrays for the control panel's Vue components, and `renderObjectTemplate()`/`renderSandboxedObjectTemplate()` source their template variables from `fields()` before falling back to raw attributes — so any object template referencing a datetime attribute (e.g. a PDF file name format or the order reference format using `dateOrdered|date(...)`) received that array instead of a `DateTime`, throwing "Array to string conversion". Every call site rendering an object template against an order now passes `$order->getObjectTemplateVariables()` in as extra variables, since `renderObjectTemplate()` won't overwrite variables that are already set.
- Added `OrderCondition::$queryParams`, restoring (at the Commerce level) the exclusive-query-param scoping the legacy Yii2 condition system provided natively via `craft\base\conditions\BaseCondition::$queryParams` — a condition instance can once again exclude rules whose `getExclusiveQueryParams()` overlaps a param it already controls outside the builder. cms-6's own condition system dropped the equivalent core mechanism in favor of rules' static `isSelectableForCondition()`, which can't express this per-instance case. `HasOrdersConditionRule::getOrderCondition()` uses it again to exclude `CustomerConditionRule` from its nested order-condition builder, since that condition is already scoped to a single customer externally.
...
- Reorganized every domain's `Records\*`/`Models\*` split under `src/` into a consistent two-namespace convention: `Models\*` is now Eloquent persistence models only (`extends CraftCms\Cms\Shared\BaseModel`), and `Data\*` holds everything else (plain data/config objects `extends CraftCms\Cms\Component\Component`), matching the existing `Catalog\ProductType\Data\ProductType`/`Order\LineItem\Data\LineItem` precedent. `Records\*` is gone — its 41 Eloquent classes moved into `Models\*`; the 46 non-Eloquent classes that previously sat in `Models\*` (`Store`, `StoreSettings`, `SiteStore`, `OrderAdjustment`, `OrderStatus`, `OrderHistory`, `OrderNotice`, `LineItemStatus`, `Transaction`, `PaymentSource`, `PaymentCurrency`, `Pdf`, `Coupon`, `Discount`, `Sale`, `PurchasableStore`, `TransferDetail`, `Email`, `TaxCategory`, `TaxRate`, `TaxAddressZone`, all `ShippingMethod`/`ShippingRule`/`ShippingCategory`/`ShippingRuleCategory`/`ShippingAddressZone`/`ShippingMethodOption` variants, all 13 Inventory movement/level classes, `CatalogPricing`, `CatalogPricingRule`, `ProductTypeSite`) moved to `Data\*`. Pure namespace rename, no behavior changes — no `class_alias` compatibility shims added, since `CraftCms\Commerce\*` is still unreleased/alpha.
- Moved `CraftCms\Commerce\Tax\Models\EuVatIdValidator` to `CraftCms\Commerce\Tax\VatValidator\Eu`, pulling it out of the `Models`/`Data` convention above since it's a stateless VAT-lookup validator (implements `TaxIdValidatorInterface`), not a data object — `TaxIdValidatorInterface` is a genuine multi-implementation extensibility point (see `TaxIdValidatorsEvent`), so it gets its own namespace rather than being forced to `extend Component`.
- Moved `craft\commerce\Plugin::_registerProjectConfigEventListeners()` to `CraftCms\Commerce\Plugin::registerProjectConfigEventListeners()`, registering directly against `app(\CraftCms\Cms\ProjectConfig\ProjectConfig::class)->onAdd()`/`onUpdate()`/`onRemove()` instead of the legacy `Craft::$app->getProjectConfig()`. Every handler method this wires up (`handleChangedGateway`, `handleChangedProductType`, `handleChangedFieldLayout`, `handleChangedOrderStatus`, `handleChangedLineItemStatus`, `handleChangedEmail`, `handleChangedStore`/`handleChangedSiteStore`, `handleChangedPdf`, and their `Deleted`/`Archived`/`Removed` counterparts) now type-hints `CraftCms\Cms\ProjectConfig\Events\ConfigEvent` instead of the legacy `craft\events\ConfigEvent` — the new `ProjectConfig::onAdd()`/`onUpdate()`/`onRemove()` invoke handlers with `ItemAdded`/`ItemUpdated`/`ItemRemoved` (all subclasses of the new `ConfigEvent`), not the legacy class. The corresponding `src-yii2/services/*` legacy shims still accept the legacy `ConfigEvent` on their own public signatures for third-party BC, but now adapt it into the new event type before delegating. Also modernized `ProductTypes::pruneDeletedSite()` to listen for the Laravel `SiteDeleted` event instead of the legacy `craft\services\Sites::EVENT_AFTER_DELETE_SITE`. The `Emails::EVENT_AFTER_DELETE_EMAIL` listener stays registered via the legacy `yii\base\Event::on()` mechanism for now (see the `TODO` in `Emails::handleDeletedEmail()` — event firing itself hasn't been bridged to Laravel yet), and the project-config-rebuild listener now uses the Laravel `ProjectConfigRebuilt` event instead of the legacy `EVENT_REBUILD`.
- Moved `craft\commerce\Plugin::_registerPoweredByHeader()` to a real Laravel middleware, `CraftCms\Commerce\Http\Middleware\PoweredByHeader`, pushed onto the `craft` middleware group (`$this->app['router']->pushMiddlewareToGroup('craft', PoweredByHeader::class)` in `Plugin::boot()`) rather than manually mutating the response headers on every boot — matching how Craft core's own equivalent, `CraftCms\Cms\Http\Middleware\SetHeaders`, already handles the `X-Powered-By: Craft CMS` half of this header via the same middleware group. Verified live: `X-Powered-By: Craft Commerce,Craft CMS`.
- Added `CraftCms\Commerce\Plugin\Concerns\HasCommerceEditions` (`$minCmsEdition`, `EDITION_PRO`/`EDITION_ENTERPRISE`/`EDITION_PRO_STORE_LIMIT`, `editions()`) and moved `$schemaVersion`/`$minVersionRequired`/`$hasCpSettings`/`$hasReadOnlyCpSettings` directly onto `CraftCms\Commerce\Plugin`, removing all of it from `craft\commerce\Plugin`.
- `craft\commerce\Plugin` is now a pure `class_alias` shim for `CraftCms\Commerce\Plugin`, matching every other fully-migrated legacy class in this migration (e.g. `Transfers`) — its custom `boot()`/method overrides and the `craft\commerce\plugin\Routes` trait it used (now deleted entirely, along with the now-empty `src-yii2/plugin/` directory) have been ported or dropped:
  - Ported `getSettingsResponse()`/`getReadOnlySettingsResponse()` (now plain `redirect('commerce/settings/general')` calls) and `createSettingsModel()` (now returns the already-ported `CraftCms\Commerce\Settings`) onto `CraftCms\Commerce\Plugin`. Also added a `getSettings(): ?Settings` override narrowing the base `HasSettings` trait's `?Validatable` return type covariantly — needed to fix 25 newly-surfaced `property.notFound` phpstan errors across `Purchasable`/`PurchasableDimensionsField`/`PurchasableWeightField`, which all call `Plugin::getInstance()->getSettings()->weightUnits`/`dimensionUnits` and only resolved correctly once the settings chain was fully wired up.
  - Ported the `relatedToProducts`/`relatedToVariants` GraphQL argument-definition injection (previously `_registerRelatedToArguments()`, hooked into the legacy `Gql::EVENT_REGISTER_GQL_QUERIES`) to listen on the modern `CraftCms\Cms\Gql\Events\GqlArgumentsResolving` event instead — the schema-level counterpart to the argument *handlers*, which were already registered via the `GqlArguments` registry in an earlier migration stage.
  - `_registerGqlInterfaces()`/`_registerGqlQueries()` were dropped entirely rather than ported — fully redundant with the already-shipped `Plugin::$gqlTypes`/`$gqlQueries` declarative arrays (`CraftCms\Cms\Plugin\Concerns\HasGql`), which register the exact same types/queries (the legacy `craft\commerce\gql\*` classes referenced are themselves just `class_alias`es of the new ones).
  - Ported the "Restore FKs" DB-repair utility registration (`_registerForeignKeysRestore()`) as-is, still a legacy `Event::on()` registration — `craft\fixfks\controllers\RestoreController` has no Laravel-native equivalent yet. Its callback now resolves the install migration via `$this->createInstallMigration()` (the same mechanism `Installable::install()` uses) instead of referencing `craft\commerce\migrations\Install` by class name directly — that class doesn't exist anymore (superseded by `database/migrations/Install.php`, `CraftCms\Commerce\Database\Migrations\Install`), which isn't PSR-4 autoloadable (Laravel migrations are `require()`'d by file path, not autoloaded).
  - Ported the bare `commerce` CP index route registration (`_registerCpRoutes()`'s one remaining rule) as-is, still a legacy `Event::on(UrlManager::class, ...)` registration — it targets a `src-yii2/templates/` Twig template that hasn't moved to `src/` yet. `_registerSiteRoutes()` was dropped entirely — it was already a fully empty, `@deprecated`-since-3.1.10 no-op.
  - Dropped `beforeInstall()`'s manual Craft-version/PHP-version guards entirely — both are now redundant with `composer.json`'s own `"php": "^8.5"` and `"craftcms/cms": "6.x-dev"` constraints, which are enforced far earlier (at `composer install` time) than this runtime check ever could be.
  - Dropped `Craft::setAlias('@commerceLib', ...)` entirely — dead code; the `lib/` directory it pointed at doesn't exist, and nothing else in the codebase references the alias.
  - **Found and dropped a pre-existing bug, not ported**: `Event::on(Purchasable::class, Elements::EVENT_BEFORE_RESTORE_ELEMENT, [$this->getPurchasables(), 'beforeRestorePurchasableHandler'])` registers a callback to a method that doesn't exist anywhere in the codebase — confirmed via `git show origin/5.x`, this is a latent bug in the currently-shipping 5.x branch too, not something introduced by this migration. Restoring a trashed purchasable element would currently throw a fatal "call to undefined method" error if this ever actually fired. Porting a guaranteed-fatal-error callback forward would only carry the bug into the new class, so it's dropped rather than ported; fixing it properly (implementing the missing handler) is out of scope here.
- Split `CraftCms\Commerce\Plugin` (previously ~990 lines) into two more `Concerns` traits, matching the already-established `HasPermissions`/`HasServices`/`HasCommerceEditions` pattern: `Plugin\Concerns\HasCommerceMacros` (all `Macroable` macro registration — Twig variables, `Site`/`User`/`Address` behavior replacements) and `Plugin\Concerns\HasCommerceEventListeners` (all `Event::listen()`/legacy `Event::on()` registration, including everything ported above). `Plugin.php` itself now only holds the class's declarative registration arrays (`$elementTypes`, `$fieldTypes`, `$events`, `$gqlTypes`, etc.), `boot()`/`register()`, and the small number of direct base-class method overrides.
- Moved the 8 listeners previously registered imperatively in `registerCraftEventListeners()` (`Login`, `Logout`, `ElementSaved`, `UserAssignedToGroups`, `SiteSaved`, `SiteDeleted`, `DefineDeletionBlockers`, `ElementAuthorizing`) into 8 new dedicated classes under `CraftCms\Commerce\Plugin\Listeners\*`, registered declaratively via `Plugin::$events` instead — each is now a normal `handle($event): void` method, matching the `ElementsHydratedListener` pattern. Since `$events`-based registration (`HasListeners::bootHasListeners()`) runs unconditionally, unlike the old method which was only called when `$this->isInstalled`, each listener's `handle()` now starts with its own `isInstalled` guard to preserve the original protection against these firing while Commerce's own tables/services aren't ready yet (e.g. during a fresh install where project config already exists).
- Replaced every `Plugin::getInstance()` call in `src/` with proper dependency injection — constructor-injected `Plugin $plugin` for container-resolved classes (controllers, the 8 listeners above, `Carts`/`Payments`/`Pdfs`), and `app(Plugin::class)` for element classes (`Product`, `Purchasable`), which aren't container-resolved and can't take constructor dependencies.
- Removed `CatalogPricing::afterSavePurchasableHandler()`, `@deprecated in 5.5.0` and unregistered anywhere in the codebase (its legacy `craft\commerce\services\CatalogPricing` pass-through wrapper is removed too).
- `CatalogPricing::generateCatalogPrices()`'s `bool $showConsoleOutput` parameter is now `?\Symfony\Component\Console\Output\OutputInterface $output`, replacing the legacy `craft\helpers\Console::stdout()` calls with `$output?->write()`. `PricingCatalogGenerateCommand` now passes its own `$this->output`; the legacy `craft\commerce\services\CatalogPricing::generateCatalogPrices()` wrapper (still typed `bool $showConsoleOutput` for its own callers) bridges `true` to a plain `new ConsoleOutput()`.
- Updated every `CraftCms\Commerce\*\Conditions\*ConditionRule` for cms-6's nestable-condition-groups rework (`craftcms/cms` PRs #19587/#19588): `ElementQueryConditionRuleInterface::modifyQuery()` now takes `(Builder $query, ElementQueryInterface $elementQuery)` instead of a single `ElementQueryInterface $query` — rules that called element-query scope setters (e.g. `OrderQuery::customerId()`) now call them on the new `$elementQuery` parameter, while rules already using `whereParam()`/`whereIn()`-style raw builder calls keep targeting `$query` (a genuine `Illuminate\Database\Query\Builder` now, not an `ElementQuery` forwarding to one). `CouponCodeConditionRule` gained its own `modifyQuery()` override to preserve case-insensitive matching that used to live in `OrderQuery::beforePrepare()`. Added `CatalogPricingCondition::createGroup()` (and a new, empty `CatalogPricingConditionGroup extends BaseConditionGroup`) — required by the new `ConditionInterface` contract for any condition not already inheriting it from `ElementCondition`; this was a hard fatal on every request once cms-6's `craftcms/cms`/`craftcms/yii2-adapter` dependencies moved past that PR. Fixed the same "`getConditionRules()` now returns a `ConditionGroupInterface` object, not an array" break in `Order::validateAddressesInMarketAddressCondition()`, `Order::modifyCustomSource()`, `Gateway::hasOrderCondition()`/`hasBillingAddressCondition()`/`hasShippingAddressCondition()`, and `CatalogPricingCondition::modifyQuery()`, all of which silently misbehaved (or, for `Gateway`, always returned `true`) rather than erroring, since PHP allows comparing/counting/iterating an object without complaint even when the code assumes an array.
- Removed `GatewayOrderCondition::getBuilderHtml()` and `Address\Conditions\GatewayAddressCondition::getBuilderHtml()` — `ConditionInterface::getBuilderHtml()` no longer exists (rendering now goes through `CraftCms\Cms\Condition\ConditionBuilderRenderer`); both classes are now empty subclasses kept only as type markers. `CatalogPricingController::filter()` and `StoreManagementController::edit()` were updated to render via `ConditionBuilderRenderer` directly.
- Ported every condition rule's custom input from the old `inputHtml()`/`elementSelectConfig()`/`inputOptions()` HTML-string methods (removed by cms-6 PR #19588) to the new Form API's `inputNodes()`, using the matching `Field`/`Control` primitives: `Textarea` (`PostalCodeFormulaConditionRule`), `ElementSelect` (`CustomerConditionRule`, `CatalogPricingCustomerConditionRule`, `CatalogPricingPurchasableConditionRule`, `CatalogPricingRulePurchasableCategoryConditionRule`), `Choice` (the purchasable-type/match-type pickers on `HasPurchasableConditionRule`/`ContainsPurchasablesConditionRule`, and the category-relationship-type picker on `CatalogPricingRulePurchasableCategoryConditionRule`), and `Money` (`OrderCurrencyValuesAttributeConditionRule`, covering `DiscountedItemSubtotalConditionRule` and the `Total*`/`Item*` order-value rules built on it). `HasPurchasableConditionRule`/`ContainsPurchasablesConditionRule`/`VariantConditionRule`/`VariantProductConditionRule`'s dropped `elementSelectConfig()` (`showSiteMenu`) is now a `elementSelect(): ElementSelect` override, matching the base class's current extension point. `HasOrdersConditionRule::getHtml()` is now `inputNodes()`, rendering its nested order-condition builder via the Form API's own `ConditionBuilder` control (`->conditionClass(OrderCondition::class)->queryParams(['customerId'])`) instead of the removed `getBuilderHtml()`. Verified by rendering each one through `ConditionBuilderRenderer` in `tests/Feature/Condition/ConditionRuleFormsTest.php`, since a broken Form API call only surfaces at render time, not through phpstan.
...
- Added `CraftCms\Commerce\Order\DeletionBlockers\OrderCustomersDeletionBlocker`, extending the new `CraftCms\Cms\Element\DeletionBlockers\BaseDeletionBlocker` instead of the legacy Yii2 one; also swaps `Craft::t()` for `t()`, `craft\helpers\Html::jsWithVars()` for `CraftCms\Cms\Support\Html::jsWithVars()`, and `craft\helpers\Cp::elementIndexHtml()` for the injectable `CraftCms\Cms\Cp\Html\ElementIndexHtml` service. Deprecated `craft\commerce\elements\deletionblockers\OrderCustomersDeletionBlocker`.
- Removed the remaining `Craft::createObject()` calls from `src/` (23 sites across 17 files) — none of them needed Yii2's DI container (interface binding, named singletons); they were all just instantiating a plain `Order`/`Address`/`OrderNotice` element or a `CraftCms\Cms\Component\Component`-based model (`Coupon`, `Discount`, `Email`, `LineItemStatus`, `OrderStatus`, `PaymentCurrency`, `Pdf`, `ShippingCategory`, `ShippingMethod`, `ShippingAddressZone`, `TaxAddressZone`, `TaxRate`, `CatalogPricingRule`) with a set of attributes. Replaced with direct `new X()` (elements, via the already-established `setAttributes()` call) or `new X([...])` (`Component`-based models, whose constructor already accepts a property config array).
- Merged `5.x` up to `5.7.3`, porting the four real fixes forward to their `src/` locations: `Order::setShippingAddress()`/`setBillingAddress()`/`setEstimatedShippingAddress()`/`setEstimatedBillingAddress()` now apply `$address['fields']` via `setFieldValues()` (previously only `CartController` did this, and only for the shipping/billing case, not estimated addresses — moving it into `Order` itself fixed the gap and let the now-redundant calls in `CartController` be removed); `Variant` regenerates its auto-generated SKU in `afterAssignedId()` when the product type's `skuFormat` references `{id}`, since that placeholder can't resolve correctly the first time SKU generation runs during `beforeSave()`, before the element has an ID; and `Store::afterSaveCraftSiteHandler()` now skips creating a site-store mapping while a project config apply is in progress, deferring to `handleChangedSiteStore()` so a site added via `craft up` doesn't trigger an unwanted follow-up config write. The CP's `index.twig` redirect logic already matched 5.7.3's `editableProductTypes` → `getViewableProductTypes()` rename (via equivalent property-style Twig access), so no change was needed there.