...
- Fixed a TODO in `craft\commerce\Plugin::_registerCraftEventListeners()`: the customer-attach bulk optimization (pre-populating `primaryBillingAddressId`/`primaryShippingAddressId` for a batch of `User` elements, so `User::getPrimaryBillingAddressId()`/`getPrimaryShippingAddressId()` — see `Plugin::registerCustomerMacros()` — don't each run their own per-user query) had been left commented out since the legacy `craft\elements\db\UserQuery::EVENT_AFTER_POPULATE_ELEMENTS` it depended on doesn't exist in Craft 6. Ported the logic to a new `CraftCms\Commerce\Customer\Listeners\ElementsHydratedListener`, listening for `CraftCms\Cms\Element\Queries\Events\ElementsHydrated` instead — the new event isn't scoped to `UserQuery` like the old one was (it fires for every element type's query), so the listener filters `$event->elements` for `User` instances itself. Registered declaratively via a new `Plugin::$events` array (`CraftCms\Cms\Plugin\Concerns\HasListeners`, already inherited by every plugin via `CraftCms\Cms\Plugin\Plugin` but not yet used by Commerce) rather than a closure in `registerCraftEventListeners()`.
...
- Added `CraftCms\Commerce\Plugin\Concerns\HasCommerceEditions` (`$minCmsEdition`, `EDITION_PRO`/`EDITION_ENTERPRISE`/`EDITION_PRO_STORE_LIMIT`, `editions()`) and moved `$schemaVersion`/`$minVersionRequired`/`$hasCpSettings`/`$hasReadOnlyCpSettings` directly onto `CraftCms\Commerce\Plugin`, removing all of it from `craft\commerce\Plugin`. Fixed a real bug found along the way: `$minCmsEdition` was initially set to `Edition::Solo` (the `HasEditions` base-trait default) rather than `Edition::Pro` — the legacy class's own `CmsEdition::Pro` override was masking this everywhere `craft\commerce\Plugin` was actually bootstrapped (i.e. everywhere except the Pest suite, which boots `CraftCms\Commerce\Plugin` directly per `tests/TestCase.php` and was silently running with the wrong minimum edition). Verified live via the still-bootstrapped `craft\commerce\Plugin` instance, which now inherits all of these values from the new class instead of shadowing them.
...
- Added `CraftCms\Commerce\Product\Variant\Elements\VariantCollection`. Deprecated `craft\commerce\elements\VariantCollection`.
