...
/**
 * OrderRecalculationTest
 *
 * Covers a real-world bug in `Order::updateOrderPaidInformation()`'s
 * recalculation lock. The scenario uses a third-party shipping method
 * plugin that registers its methods dynamically via
 ...
 * match is normal cart behaviour and not the bug on its own (see
 * {@see \craftcommercetests\unit\adjusters\ShippingTest::testAdjusterDropsAdjustmentWhenMethodDoesNotMatch()}).
 *
 * The real bug: this could happen to an order that had *already completed
 * and been paid for*. `updateOrderPaidInformation()` locks
 ...
 */
class OrderRecalculationTest extends Unit
{
    /**
     * @var UnitTester
     */
