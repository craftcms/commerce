...
class m150418_125314_market_OrderHistory extends BaseMigration
{
    ...
    public function safeUp()
    {
        ...
        craft()->db->createCommand()->addForeignKey('market_orderhistories',
            'orderId', 'market_orders', 'id', 'CASCADE', 'CASCADE');
        craft()->db->createCommand()->addForeignKey('market_orderhistories',
            'prevStatusId', 'market_orderstatuses', 'id', 'RESTRICT',
            'CASCADE');
        craft()->db->createCommand()->addForeignKey('market_orderhistories',
            'newStatusId', 'market_orderstatuses', 'id', 'RESTRICT', 'CASCADE');
        craft()->db->createCommand()->addForeignKey('market_orderhistories',
            'userId', 'users', 'id', 'RESTRICT', 'CASCADE');
