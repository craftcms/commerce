 ...
 * @since     1.0
 */
class Commerce_PaymentService extends BaseApplicationComponent
{
	/**
	...
	)
	{
		...
		if ($cart->isPaid())
		{
			craft()->commerce_order->complete($cart);
		...
		$transaction = craft()->commerce_transaction->create($cart);
		...
		try
		{
			...
			{
				craft()->commerce_order->updateOrderPaidTotal($cart);
			}
		}
		catch (\Exception $e)
		{
			$customError = $e->getMessage();
			craft()->commerce_transaction->delete($transaction);
	...
	)
	{
		...
		$order = $parent->order;
		$child = craft()->commerce_transaction->create($order);
	...
	public function completePayment (Commerce_TransactionModel $transaction)
	{
		...
		if ($gateway->$supportsAction())
		{
			...
			if ($transaction->status == Commerce_TransactionRecord::SUCCESS)
			{
				craft()->commerce_order->updateOrderPaidTotal($order);
			}
			craft()->request->redirect($redirect);
		}
		else
	...
	)
	{
		...
		if ($response->isSuccessful())
		{
			craft()->commerce_order->updateOrderPaidTotal($transaction->order);
		}
	...
	{
		if (!craft()->commerce_transaction->save($child))
		{
