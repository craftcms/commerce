...
class Cp
{
    public static function inventoryLocationFieldHtml(array $config): string
    {
        ...
        return FormFields::fieldHtml('template:commerce/_includes/forms/inventoryLocationSelect.twig', $config);
    }

    public static function taxZoneFieldHtml(array $config): string
    {
        $config['id'] ??= 'taxzoneselect' . mt_rand();
        return FormFields::fieldHtml('template:commerce/_includes/forms/taxZoneSelect.twig', $config);
    }

    public static function taxCategoryFieldHtml(array $config): string
    {
        $config['id'] ??= 'taxcategoryselect' . mt_rand();
        return FormFields::fieldHtml('template:commerce/_includes/forms/taxCategorySelect.twig', $config);
    }

    public static function shippingCategoryFieldHtml(array $config): string
    {
        $config['id'] ??= 'shippingcategoryselect' . mt_rand();
        return FormFields::fieldHtml('template:commerce/_includes/forms/shippingCategorySelect.twig', $config);
    }
}
