...
/**
 * Class Commerce_CartAddressController
 *
 ...
 * @since     1.0
 */
class Commerce_CartAddressController extends Commerce_BaseFrontEndController
{
	...
	public function actionPostTwoAddresses ()
	{
		...
		if (craft()->commerce_order->setAddresses($order, $shipping, $billing))
		{

			craft()->commerce_customer->setLastUsedAddresses($billing->id, $shipping->id);
	...
	public function actionChooseAddresses ()
	{
		...
		$billingAddress = craft()->commerce_address->getAddressById($billingId);
		...
		else
		{
			...
			$shippingAddress = craft()->commerce_address->getAddressById($shippingId);
		}
		...
		$customerId = craft()->commerce_customer->getCustomerId();
		$addressIds = craft()->commerce_customer->getAddressIds($customerId);

		if (in_array($billingAddress->id, $addressIds) && in_array($shippingAddress->id, $addressIds))
		{
			if (craft()->commerce_order->setAddresses($order, $shippingAddress, $billingAddress))
			{
				craft()->commerce_customer->setLastUsedAddresses($billingAddress->id, $shippingAddress->id);
				if (craft()->request->isAjaxRequest)
				{
					$this->returnJson(['success' => true, 'cart' => $this->cartArray($order)]);
				}
	...
	public function actionAddAddress ()
	{
		...
		if (craft()->commerce_customer->saveAddress($address))
		{
			if (craft()->request->isAjaxRequest)
			{
	...
	public function actionRemoveAddress ()
	{
		...
		{
			if (craft()->commerce_address->deleteAddressById($id))
			{
				if (craft()->request->isAjaxRequest)
				{
					$this->returnJson(['success' => true]);
