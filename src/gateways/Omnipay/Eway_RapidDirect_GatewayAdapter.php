...
class Eway_RapidDirect_GatewayAdapter extends \Commerce\Gateways\CreditCardGatewayAdapter
{
    ...
    public function getPaymentFormHtml(array $params)
    {
        ...
        $csekey = isset($this->getPaymentMethod()->settings['CSEKey']) && $this->getPaymentMethod()->settings['CSEKey'];
        if ($csekey) {
            if (\Craft\Craft::$app->getConfig()->get('devMode')) {
                \Craft\Craft::$app->getView()->includeJsFile('https://secure.ewaypayments.com/scripts/eCrypt.debug.js');
            } else {
                \Craft\Craft::$app->getView()->includeJsFile('https://secure.ewaypayments.com/scripts/eCrypt.js');
            }
        }
        ...
        if ($csekey) {
            return \Craft\Craft::$app->getView()->render('commerce/_gateways/_paymentforms/ewayrapiddirectencrypt', $params);
        } else {
