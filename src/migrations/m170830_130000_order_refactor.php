...
class m170830_130000_order_refactor extends Migration
{
    ...
    public function safeUp(): bool
    {
        ...
        // Grab all orders.
        $allOrders = (new Query())->select('*')->from('{{%commerce_orders}}');
        ...
        foreach ($allOrders->batch() as $orders) {
            foreach ($orders as $order) {
                ...
                $lineItems = (new Query())->select('*')->from('{{%commerce_lineitems}}')->where(['orderId' => $order['id']])->all();
                $adjustments = (new Query())->select('*')->from('{{%commerce_orderadjustments}}')->where(['orderId' => $order['id']])->all();
