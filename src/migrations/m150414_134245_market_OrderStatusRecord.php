...
class m150414_134245_market_OrderStatusRecord extends BaseMigration
{
	...
	public function safeUp()
	{
        ...
        craft()->db->createCommand()->addForeignKey('market_orderstatuses', 'orderTypeId', 'market_ordertypes', 'id', null, null);

        // Create the craft_market_orderstatus_emails table
        craft()->db->createCommand()->createTable('market_orderstatus_emails', array(
            'orderStatusId' => array('column' => 'integer', 'required' => true),
            'emailId'       => array('column' => 'integer', 'required' => true),
        ), null, true);

        // Add foreign keys to craft_market_orderstatus_emails
        craft()->db->createCommand()->addForeignKey('market_orderstatus_emails', 'orderStatusId', 'market_orderstatuses', 'id', 'CASCADE', 'CASCADE');
        craft()->db->createCommand()->addForeignKey('market_orderstatus_emails', 'emailId', 'market_emails', 'id', 'CASCADE', 'CASCADE');
