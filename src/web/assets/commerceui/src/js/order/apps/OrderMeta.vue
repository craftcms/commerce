<template>
    <div
        class="order-meta-container"
        :class="{loading: recalculateLoading || saveLoading}"
        v-if="draft"
    >
        <div id="settings" class="meta" v-if="editing">
            <div class="field" id="reference-field">
                <div class="heading">
                    <label id="reference-label" for="reference">{{
                ...
                <div class="input ltr">
                    <input
                        ...
                        autocapitalize="off"
                        :placeholder="
                            $options.filters.t('Enter reference', 'commerce')
                        "
                    />
                </div>
            </div>
            ...
            <field
                :label="$options.filters.t('Coupon Code', 'commerce')"
            ...
            >
                <input
                    ...
                    autocapitalize="off"
                    :placeholder="
                        $options.filters.t('Enter coupon code', 'commerce')
                    "
                />
            </field>
            ...
            <field
                ...
                :label="$options.filters.t('Date Ordered', 'commerce')"
            >
                <date-ordered-input
                    :date="draft.order.dateOrdered"
                    @update="updateDateOrderedInput"
                ></date-ordered-input>
            </field>
            ...
            <div
                class="field"
                id="isCompleted-field"
                v-if="!draft.order.isCompleted"
            >
                <div class="heading">
                    <label id="isCompleted-label" for="isCompleted">{{
            ...
            <template v-if="draft.order.isCompleted">
                <div class="field" id="orderStatus-field">
                    <div class="heading">
                        <label id="orderStatus-label" for="status">{{
                            'Status' | t('commerce')
                        }}</label>
                    </div>
                    <div class="input ltr">
            ...
            <div class="field" id="shippingMethod-field">
                <div class="heading">
                    <label id="shippingMethod-label" for="slug">{{
                        'Shipping Method' | t('commerce')
                    }}</label>
                </div>
                <div class="input ltr">
                    <shipping-method
        ...
        <div class="order-meta meta read-only" v-if="!editing">
            ...
            <template v-if="draft.order.isCompleted">
                <div class="data">
                    <h5 class="heading">
                        {{ 'Date Ordered' | t('commerce') }}
                    </h5>
                    <span class="value"
                        >{{ draft.order.dateOrdered.date }}
                        {{ draft.order.dateOrdered.time }}</span
                    >
                </div>
            </template>
            ...
            <div class="data">
                ...
                <span class="value" v-if="draft.order.orderSite"
                    >{{ draft.order.orderSite.name }} ({{
                        draft.order.orderSite.language
                    }})</span
                >
            </div>
            ...
            <template v-if="draft.order.isCompleted">
                <div class="data">
                    ...
                    <span
                        class="value"
                        v-html="draft.order.orderStatusHtml"
                    ></span>
                </div>
            </template>
            ...
            <div class="data order-meta-shipping-method">
                ...
                <div class="value" v-if="draft.order.shippingMethodHandle">
                    <span v-if="draft.order.shippingMethodName">{{
                        draft.order.shippingMethodName
                    }}</span>
                    <span class="small code shipping-method-handle">{{
                        draft.order.shippingMethodHandle
                    }}</span>
                </div>
            </div>
        </div>
        ...
        <div id="meta" class="meta read-only">
            ...
            <div class="data">
                ...
                <div class="value order-number-value">
                    ...
                    <div class="hidden-input">
                        <input
                            type="text"
                            ref="orderShortNumber"
                            :value="draft.order.shortNumber"
                        />
                    </div>
                    <btn-link @click="copy($refs.orderShortNumber)">{{
                        'Copy' | t('commerce')
                    }}</btn-link>
                </div>
            </div>
            ...
            <div class="data">
                ...
                <div class="value order-number-value">
                    ...
                    <btn-link @click="copy($refs.orderNumber)">{{
                        'Copy' | t('commerce')
                    }}</btn-link>
                </div>
            </div>
            ...
            <div class="data">
                ...
                <span class="value">{{
                    draft.order.totalPriceAsCurrency
                }}</span>
            </div>

            <template v-if="draft.order.totalPaid != 0">
                <!-- Show positive and negative numbers. -->
                <div class="data">
                    <h5 class="heading">{{ 'Paid Amount' | t('commerce') }}</h5>
                    <span class="value">{{
                        draft.order.totalPaidAsCurrency
                    }}</span>
                </div>
            </template>

            <template
                v-if="!draft.order.datePaid && draft.order.dateAuthorized"
            >
                <div class="data">
                    <h5 class="heading">
                        {{ 'Date Authorized' | t('commerce') }}
                    </h5>
                    <span class="value"
                        >{{ draft.order.dateAuthorized.date }}
                        {{ draft.order.dateAuthorized.time }}</span
                    >
                </div>
            </template>
            ...
            <template v-if="draft.order.datePaid">
                <div class="data">
                    ...
                    <span class="value"
                        >{{ draft.order.datePaid.date }}
                        {{ draft.order.datePaid.time }}</span
                    >
                </div>
            </template>
            ...
            <div class="data">
                ...
                <span class="value"
                    ...
                    {{ draft.order.dateUpdated.time }}</span
                >
            </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            ...mapGetters(['getErrors', 'hasCustomer']),
            ...
            shippingMethod() {
                ...
                let shippingMethod = shippingMethods.find(
                    (method) => method.handle && method.handle == handle
                );
        ...
        methods: {
            ...
            copy(ref) {
                ...
                this.$store.dispatch(
                    'displayNotice',
                    this.$options.filters.t('Copied!', 'commerce')
                );
            },
